  ...
  try {
    ...
    recipientList.forEach(item => {
      ...
      if (email && email.includes('@')) {
        try {
          sendSesEmail(email, subject, bodyHtml, accessKey, secretKey, region);
          successCount++;
        } catch (err) {
 ...
 */
function sendSesEmail(toAddress, subject, htmlBody, accessKey, secretKey, region) {
  const method = 'POST';
  const service = 'email'; 
  const host = `email.${region}.amazonaws.com`;
  const sender = PropertiesService.getScriptProperties().getProperty('SOURCE_EMAIL');
  
  if (!sender) throw new Error("SOURCE_EMAIL property is missing.");
